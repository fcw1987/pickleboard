...
const validReport = () => ({
  suites: [{ specs: [...Array.from({ length: 181 }, (_, i) => passingSpec(i)), ...ALLOWED_FAILURES.map(failingSpec)] }],
  errors: [], stats: { expected: 181, unexpected: 7, flaky: 0, skipped: 0 },
});
...
test('rejects an unknown error signature', () => {
  ...
  report.suites[0].specs[181].tests[0].results[1].errors[0].message = 'Error: page.reload: a different failure';
  expectRejected(inspect(report), /wrong error signature/);
});

test('rejects an unexpected pass of an allowed failure', () => {
  const report = validReport();
  report.suites[0].specs[181].tests[0] = { projectName: 'webkit', expectedStatus: 'passed', status: 'expected', results: [{ status: 'passed', errors: [] }] };
...
test('rejects timed out and skipped results', () => {
  ...
  timedOut.suites[0].specs[181].tests[0].results[0] = result(ALLOWED_FAILURES[0], 'timedOut');
...
test('rejects an extra assertion error on an allowed failure', () => {
  const report = validReport();
  report.suites[0].specs[181].tests[0].results[0].errors.push({ message: 'extra', location: { file: ALLOWED_FAILURES[0].file, line: 125 } });
  expectRejected(inspect(report), /expected one error/);
});

test('rejects a wrong failure line', () => {
  const report = validReport();
  report.suites[0].specs[181].tests[0].results[0].errors[0].location.line += 1;
