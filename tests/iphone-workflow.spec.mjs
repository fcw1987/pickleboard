...
test('invalid numeric edits and import remain recoverable inside the phone controls',async({page})=>{
 ...
 await page.evaluate(()=>playBuilder.action('editShot',{field:'target.x',value:5.999999791}));await details(page);await expect(page.getByLabel('Target width',{exact:true})).toHaveValue('6');await finish(page);expect(await page.evaluate(()=>playBuilder.selectedShot.target.x)).toBe(5.999999791);await menu(page,'File');await page.getByRole('button',{name:'Import / Export',exact:true}).click();await page.getByLabel('JSON backup').fill('{broken');await page.getByRole('button',{name:'Import JSON',exact:true}).click();await expect(page.getByRole('dialog')).toBeVisible();await expect(page.getByLabel('JSON backup')).toHaveValue('{broken');await expect(page.locator('.builder-import-status')).toContainText('malformed');
