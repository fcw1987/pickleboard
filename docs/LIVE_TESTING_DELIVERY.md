...
## First deployment guard and correction

The first manual deployment [34065134046](https://github.com/fcw1987/pickleboard/actions/runs/34065134046) stopped before publication because the official Pages upload action excludes dotfiles, including the unnecessary `.nojekyll` marker. The complete path-set guard correctly rejected that difference. The marker is now removed from the publication metadata contract; Actions-based deployment does not run Jekyll. All 81 application files remain unchanged. Quality now verifies the actual tar produced by the official action as well as the served staging directory, using the same tested validator as deployment. The previous live site was not replaced by this failed attempt.

Merged sanitized main `0ee64a1` passed [Quality run 34064867643](https://github.com/fcw1987/pickleboard/actions/runs/34064867643): 71 unit tests, 77 static references, brand and 62 generated outputs, 122 Chromium tests, and all four fresh/production-upgrade package cases. WebKit reported 115 passes and seven strictly matched infrastructure failures with an independent Linux reproduction; see issue 14. Subsequent release-tooling revisions repeat the complete gate before deployment.
